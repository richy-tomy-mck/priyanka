      ...
      <FlatList
        ...
        ListHeaderComponent={() => (
          <>
            <View style={styles.container}>
              ...
              <Text style={styles.noOfSearchedJobs}>{`${((page - 1) * limit)+1} - ${
                orders.length < (page * limit)? orders.length : page * limit
              } of ${orders.length}`}</Text>
            </View>
            <View style={styles.loaderContainer}>
              {fetchLoader ? (
                <ActivityIndicator size="large" color={COLORS.primary} />
              ) : (
                fetchError && <Text>Oops something went wrong</Text>
              )}
            </View>
          </>
        )}
        ListFooterComponent={() => (
          <View style={styles.footerContainer}>
            ...
            {(page * limit < orders.length || !limitReached) && (
              <TouchableOpacity
                style={styles.paginationButton}
